...
| Skill | 用途 |
|-------|------|
| `dep-vulnerability-check` | パッケージインストール時の脆弱性チェック（MUST reference） |
...
| Skill | 用途 |
|-------|------|
| `chrome-devtools-mcp` | DevTools MCP 接続・トラブルシューティング・操作パターン（MUST reference when using DevTools MCP） |
| `browser-image-download` | Chrome DevTools MCP 経由で画像をダウンロード（一時ファイルコピー方式） |

### コンテンツ制作

| Skill | 用途 |
|-------|------|
| `tiktok-slideshow` | TikTok スライドショー投稿の企画・画像生成・テキスト合成 |
