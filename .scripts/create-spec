...
# 最適化されたコードベース分析（CLAUDE.md/.cursorrules優先）
analyze_codebase_smart() {
    echo -e "${MAGENTA}🧠 リポジトリを効率的に分析中（CLAUDE.md/.cursorrules優先）...${NC}\n"

    local all_analysis=""

    for repo_path in "${REPOSITORIES[@]}"; do
        local repo_name=$(basename "$repo_path")
        echo -e "${CYAN}📦 リポジトリ: ${repo_name}${NC}"

        local context_found=false
        local analysis_result=""

        # Step 1: CLAUDE.md を探す（最優先）
        if [ -f "$repo_path/CLAUDE.md" ]; then
            echo -e "${GREEN}   ✓ CLAUDE.md 検出（高速読み込み）${NC}"
            analysis_result="## プロジェクト概要（CLAUDE.mdより）\n\n\`\`\`markdown\n$(cat "$repo_path/CLAUDE.md")\n\`\`\`"
            context_found=true
        # Step 2: .cursorrules を探す
        elif [ -f "$repo_path/.cursorrules" ]; then
            echo -e "${GREEN}   ✓ .cursorrules 検出（高速読み込み）${NC}"
            analysis_result="## プロジェクト概要（.cursorrulesより）\n\n\`\`\`\n$(cat "$repo_path/.cursorrules")\n\`\`\`"
            context_found=true
        fi

        # Step 3: 軽量分析（package.json, README等）
        if [ "$context_found" = false ]; then
            echo -e "${YELLOW}   ⚡ 軽量コンテキスト読み込み中...${NC}"
            local light_context=""

            # ディレクトリ構造の簡易確認
            if [ -d "$repo_path" ]; then
                light_context="${light_context}\n### ディレクトリ構造（トップレベル）\n\`\`\`\n$(ls -1 "$repo_path" | head -15)\n\`\`\`\n"
            fi

            # package.json or pyproject.toml or go.mod etc
            if [ -f "$repo_path/package.json" ]; then
                light_context="${light_context}\n### package.json\n\`\`\`json\n$(head -20 "$repo_path/package.json")\n\`\`\`\n"
            elif [ -f "$repo_path/go.mod" ]; then
                light_context="${light_context}\n### go.mod\n\`\`\`\n$(cat "$repo_path/go.mod")\n\`\`\`\n"
            elif [ -f "$repo_path/pyproject.toml" ]; then
                light_context="${light_context}\n### pyproject.toml\n\`\`\`toml\n$(head -20 "$repo_path/pyproject.toml")\n\`\`\`\n"
            elif [ -f "$repo_path/Cargo.toml" ]; then
                light_context="${light_context}\n### Cargo.toml\n\`\`\`toml\n$(head -20 "$repo_path/Cargo.toml")\n\`\`\`\n"
            fi

            # README
            if [ -f "$repo_path/README.md" ]; then
                light_context="${light_context}\n### README.md (抜粋)\n\`\`\`markdown\n$(head -30 "$repo_path/README.md")\n\`\`\`\n"
            fi

            analysis_result="## 軽量コンテキスト\n${light_context}\n\n**【注記】** 詳細な分析が必要な場合は、対話中に「\`${repo_name}\`の詳細を確認したい」と入力してください。"
            echo -e "${GREEN}   ✓ 軽量コンテキスト読み込み完了${NC}"
        fi
    ...
    echo -e "${CYAN}=== 全リポジトリのコンテキスト読み込み完了 ===${NC}"
    echo -e "${SPEC_DATA[claude_analysis]}"
    echo ""
}

# オンデマンドでリポジトリを詳細分析（Serena MCP）
explore_repository_on_demand() {
    local repo_name=$1
    local repo_path=""

    # リポジトリパスを検索
    for rp in "${REPOSITORIES[@]}"; do
        if [ "$(basename "$rp")" = "$repo_name" ]; then
            repo_path="$rp"
            break
        fi
    done
    ...
    if [ -z "$repo_path" ]; then
        echo -e "${RED}✗ リポジトリが見つかりません: ${repo_name}${NC}"
        return 1
    fi

    echo -e "${MAGENTA}🔍 ${repo_name} の詳細分析を実行中（Serena MCP）...${NC}\n"
...
2. **コードベース構造分析**
   - 主要なシンボル（関数、クラス、インターフェース）
   - アーキテクチャパターン
   - 主要な機能とディレクトリ構造

3. **分析結果の記憶**
   - \`create_memory\` または \`update_memory\` で保存
   - メモリ名: \`${repo_name}-detailed-analysis\`
...
EOF
    ...
    claude --print "$(cat "$PROMPT_FILE")" > "$CLAUDE_RESPONSE_FILE" 2>&1 || {
        echo -e "${RED}✗ 詳細分析に失敗しました${NC}"
        cat "$CLAUDE_RESPONSE_FILE"
        return 1
    }

    local detailed_result=$(cat "$CLAUDE_RESPONSE_FILE")

    # 既存の分析結果を更新
    REPO_ANALYSIS["$repo_name"]="${REPO_ANALYSIS[$repo_name]}\n\n## 詳細分析結果（Serena MCP）\n\n${detailed_result}"

    # 統合分析結果も更新
    SPEC_DATA[claude_analysis]=$(echo -e "${SPEC_DATA[claude_analysis]}" | sed "s|### リポジトリ: ${repo_name}.*|### リポジトリ: ${repo_name} (${repo_path})\n\n${REPO_ANALYSIS[$repo_name]}\n\n---|")

    echo -e "${GREEN}✓ ${repo_name} の詳細分析完了${NC}\n"
    echo -e "${detailed_result}"
    echo ""
}
...
**【重要】軽量コンテキストでの質問生成について:**
- 現在のコンテキストは軽量版（CLAUDE.md/.cursorrules優先、または基本情報のみ）です
- 軽量コンテキストでも答えられる質問を優先してください
- 詳細なコード構造の確認が必要な場合は、「〇〇リポジトリの△△について詳細確認が必要」と明示してください
- ユーザーが詳細確認を希望した場合、該当リポジトリをSerena MCPで詳細分析します
...
interactive_qa_loop() {
    ...
    while [ $iteration -le $max_iterations ]; do
        ...
        while [ $question_count -le 3 ]; do
            ...
            echo "  3) 後で決める"
            echo "  4) スキップ"
            echo "  5) リポジトリの詳細を確認したい（Serena MCP分析）"
            echo -n "> "
            ...
            case $choice in
                ...
                4)
                    ...
                    ;;
                5)
                    echo -e "${CYAN}詳細分析したいリポジトリ名を入力してください:${NC}"
                    echo -n "> "
                    read -r repo_name

                    # リポジトリの詳細分析を実行
                    explore_repository_on_demand "$repo_name"

                    # 分析後、改めて回答を入力
                    echo -e "${CYAN}分析結果を踏まえて、質問${question_count}への回答を入力してください (空行で終了):${NC}"
                    local lines=""
                    local first_line=true
                    while IFS= read -r line; do
                        [ -z "$line" ] && break
...
main() {
    ...
    echo -e "${BLUE}=== ステップ2: コードベース分析（最適化版）===${NC}\n"
    analyze_codebase_smart
